...
> [!IMPORTANT]
> 当前客户端静态等级数据最高只到 `1400`。如果手动改成 `4500`、`9999` 这类数值，客户端可能在加载界面卡住。这个 fork 不修改游戏资源，所以安全上限就是 `1400`。
...
- 推荐等级：`1400`
- 不推荐：`4500`、`9999` 或更高
- 如果数据库里已经有超过 `1400` 的等级，启动服务端后会自动压回 `1400`

这样做的目的不是限制玩法，而是避免客户端读取不存在的等级表数据后无限加载。
